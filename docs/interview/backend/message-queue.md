...
- **异步**：让不影响当前响应的工作稍后完成；
- **解耦**：生产者不必同步调用每个下游；
- **削峰**：队列吸收短时流量，消费者按可承受速度处理。

代价是系统要处理**消息丢失、重复、乱序、积压和最终一致性**。如果调用方必须立即取得结果，或者异步后的补偿成本不可接受，RPC 可能更合适。

## 消息队列和异步任务是什么关系？
{: #message-queue-vs-async-task }

- **异步任务**：表示一项工作需要跨越当前请求继续存在；
- **消息队列**：是把待处理工作交给消费者的一种传输和调度机制。
...
- **At Most Once**：最多处理一次，可能丢失，但通常不会重做；
- **At Least Once**：通过确认和重试减少永久丢失，但消息可能重复；
- **Exactly Once**：希望最终效果只出现一次，实际保证范围取决于参与的系统边界。

生产系统常按 **At Least Once + 业务幂等**设计，使重复消息只产生一份业务结果。
...
1. **业务提交后、发送前**：应用在数据库提交后崩溃，消息尚未发送；
2. **生产传输中**：请求或确认丢失，而生产者没有正确重试；
3. **Broker 内部**：消息在持久化或复制完成前丢失；
4. **消费阶段**：业务尚未处理成功，消息却被提前确认。
...
消费者应优先使用订单号、支付请求 ID、事件 ID 等稳定业务标识，并在真正产生业务结果的位置阻止重复：

- **唯一约束**：同一业务标识只能创建一次结果；
- **条件状态转换**：只有满足前置状态时才更新；
- **同一事务记录消费**：消费记录与业务写入共同提交；
- **结果查询**：重复到达时返回已经存在的结果。

只在 Redis 中短暂记录“已经消费”可能因过期、淘汰或写入时序重新放出重复。
...
- **延迟消息**：由消息系统在指定时间后交付，适合规模较大、以事件触发的延后处理；
- **定时任务**：由调度器按时间规则扫描或创建工作，更适合周期任务和可查询的业务期限。
...
1. **确认现状**：检查生产速度、消费速度、积压量和最老消息年龄；
2. **定位瓶颈**：排查流量突增、慢 SQL、下游变慢、锁竞争、线程池不足或 Partition 数限制；
3. **止损与恢复**：限制上游、暂停非关键生产，并在业务允许并行时增加消费者。

只增加实例未必有效；消费者数量超过可并行队列数，或者下游已经饱和，扩容只会增加竞争。
...
- **Topic**：消息的逻辑分类；
- **Partition**：Topic 的有序追加日志，也是并行单位；
- **Broker**：保存 Partition 并服务读写的 Kafka 节点；
- **Replica**：同一 Partition 在不同 Broker 上的副本。
...
- **`session.timeout`**：结合心跳判断消费者是否仍然存活；
- **`max.poll.interval`**：限制两次 Poll 之间允许的最长处理时间，防止消费者活着却长时间不继续拉取。
...
- **Leader**：负责 Partition 的客户端读写；
- **Follower**：从 Leader 复制日志；
- **ISR（In-Sync Replicas）**：当前与 Leader 保持足够同步、具备接管资格的副本集合。
...
- **Kafka**：适合高吞吐事件流、日志保留、重放和多个消费组独立读取；
- **RabbitMQ**：更偏传统消息代理，擅长灵活路由、单条消息投递和任务队列语义。

选择应依据**消息保留与重放、顺序、路由、吞吐、延迟和团队运维能力**，不应只比较单项性能数字。
