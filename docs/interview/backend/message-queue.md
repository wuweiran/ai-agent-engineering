...
Outbox 把业务数据和“需要发送的事件”写入同一个本地数据库事务。独立发布器随后发送待处理事件，因此应用在业务提交后崩溃，也不会永久忘记发送。

发布器可能已经发送成功，却在标记事件前崩溃，所以 Outbox 通常把“消息不丢”转换成“消息可能重复”，消费者仍要幂等。
...
常见方案是 Transactional Outbox：业务事实与待发送事件共同提交，再由发布器可靠发送。若业务与消息系统支持经过验证的事务消息，也要明确其回查和故障边界。
...
队列长度下降只是传输侧信号。还要确认新消息延迟恢复、最老积压持续下降、死信得到处理，并且订单、库存等业务结果重新满足承诺。

清理历史积压的速度应低于系统剩余容量，避免刚恢复的下游再次被压垮。
...
这主要覆盖 Kafka 到 Kafka 的处理链路。写外部数据库、调用支付服务等副作用不会自动加入 Kafka 事务，仍需 Outbox、幂等或其他协调机制。
