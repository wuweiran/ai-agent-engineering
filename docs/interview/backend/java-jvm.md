...
- **进程**：拥有独立地址空间和系统资源，是操作系统隔离程序的主要边界。进程间通信需要管道、Socket 或共享内存等机制，隔离强、创建和切换成本也较高；
- **线程**：属于进程，共享堆和打开的资源，但各自拥有栈和执行状态，通常由操作系统调度。线程通信方便，也更容易发生数据竞争；
- **协程**：由语言运行时或库管理的可暂停任务，通常把大量协程复用到较少线程上，在 `await`、`yield` 等挂起点让出执行权。创建和切换较轻，但阻塞调用或长时间 CPU 计算仍可能占住底层线程。

三者不在完全相同的抽象层：进程提供资源隔离，线程提供进程内并行执行，协程主要提高大量等待型任务的并发效率。协程本身不能增加 CPU 核数，也不能替代进程隔离。

Java 虚拟线程同样由 JVM 调度并复用平台线程，目标也是降低大量阻塞任务的线程成本，但它保留了线程式编程模型，不应把所有语言中的协程和 Java 虚拟线程视为同一种实现。
...
`ThreadPoolExecutor` 的核心参数包括 `corePoolSize`、`maximumPoolSize`、`keepAliveTime`、`workQueue`、`threadFactory` 和 `RejectedExecutionHandler`。任务提交顺序是：

1. 运行线程少于核心线程数时，创建线程执行；
2. 核心线程已满时，先尝试进入工作队列；
3. 队列已满且线程数小于最大线程数时，再创建非核心线程；
4. 队列和最大线程都达到上限时，执行拒绝策略。

因此不同队列会改变 `maximumPoolSize` 是否生效。使用无界队列时，任务通常持续排队，线程池很可能不会扩展到最大线程数，过载最终表现为队列增长、延迟升高和内存压力。

JDK 常见拒绝策略包括：`AbortPolicy` 抛出异常，`CallerRunsPolicy` 由提交任务的线程执行以减慢生产者，`DiscardPolicy` 静默丢弃，`DiscardOldestPolicy` 丢弃队列中最旧任务后重试。关键业务通常不应静默丢弃，应选择可观测的拒绝或明确的背压方式。
...
相关内容：[容量、排队与过载保护]({{ site.baseurl }}/docs/backend/capacity-backpressure/)。
...
**原理：**`@Transactional` 提供事务元数据，Spring 代理拦截方法调用，再委托事务管理器完成以下工作：

1. 从连接池取得数据库连接，关闭自动提交，并设置隔离级别、只读等属性；
2. 将连接绑定到当前线程，使本次调用中的 Repository 共用同一事务连接；
3. 执行业务方法，正常结束时 `commit`，异常符合回滚规则时 `rollback`；
4. 解除线程绑定，恢复连接状态并归还连接池。

对于常见关系数据库访问，Spring 事务底层仍通过 JDBC `Connection` 控制本地事务；使用 JPA 或 MyBatis 时，只是上层访问方式不同，最终仍要落到数据库连接及其 `commit`、`rollback`。事务内的数据库操作需要使用当前事务关联的同一连接。

Spring 主要替应用管理代理、连接传递、提交回滚、传播行为和资源清理；真正实施原子提交与回滚的仍是数据库。
...
相关内容：[服务调用中的超时与容错]({{ site.baseurl }}/docs/backend/service-timeouts/)、[容量、排队与过载保护]({{ site.baseurl }}/docs/backend/capacity-backpressure/)。
