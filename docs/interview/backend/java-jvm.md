...
1. **编译**：Java 编译器把源码编译成平台无关的字节码；
2. **加载**：类加载器按需把类加载、连接并初始化到 JVM；
3. **执行**：解释器可以立即运行字节码，热点代码再由 JIT 编译成本地机器码。

因此 Java 通常被称为**编译与解释并存**。AOT 可以提前生成本地代码，减少启动和预热时间，但会减少一部分运行时画像驱动的优化，并增加构建与平台适配成本。
...
- **基本类型**：直接表示数值或布尔值，不能为 `null`；
- **包装类型**：属于对象，可用于泛型和集合，但会增加对象和空值处理成本；
- **自动装箱与拆箱**：编译器在基本类型和包装类型之间自动转换。

常见风险包括：对 `null` 包装类型拆箱会抛出 `NullPointerException`；频繁拆装箱可能产生额外分配；部分包装值存在缓存，因此**不能用 `==` 判断包装对象的数值相等**。
...
- **record**：适合表达以数据为主的浅不可变载体；
- **sealed class**：限制允许的继承层次；
- **模式匹配**：减少类型判断和转换的样板代码；
- **文本块**：简化多行字符串；
- **虚拟线程**：降低大量阻塞任务的线程成本。

**虚拟线程不会提高数据库、下游服务和 CPU 的真实容量**，也不应与平台线程池的调参方法直接混为一谈。
...
- **List**：保留元素顺序并允许重复；
- **Set**：表达唯一元素集合；
- **Queue / Deque**：表达排队或双端操作；
- **Map**：保存 Key 到 Value 的映射。

选择时先看**业务语义**，再看访问模式、顺序、并发和内存。集合接口表达需要什么能力，具体实现决定性能与线程安全。
...
1. **查询**：根据 Key 的哈希值定位数组中的桶，再通过哈希值和 `equals` 确认 Key；
2. **冲突处理**：不同 Key 落入同一桶时形成冲突，现代 JDK 在极端情况下可能把链式结构转换为红黑树；
3. **扩容**：元素超过负载条件后，创建更大的桶数组并重新分配节点。

合理初始容量可以减少扩容，但**树化阈值和扩容细节属于 JDK 实现，不是 Map 接口保证**。
...
**有界队列**还能把下游容量反馈给生产者；**无界队列**则可能把过载隐藏成持续积压。

- **ArrayBlockingQueue**：使用固定数组，容量创建后不变；
- **LinkedBlockingQueue**：使用链式节点，可以显式设置容量。
...
- **`synchronized`**：语言内置的互斥与可见性机制，进入和退出代码块时自动获取、释放监视器；
- **`Lock`**：显式锁接口，可以支持可中断等待、超时获取、多个条件队列和公平策略。
...
- **锁规则**：解锁先于后来对同一锁的加锁；
- **volatile 规则**：写先于后来对同一变量的读；
- **线程启动**：调用 `start` 先于新线程中的动作；
- **线程结束**：线程中的动作先于其他线程检测到它终止。

没有这些关系时，代码不能依赖某次写入一定被其他线程及时看到。

## CAS 是什么？ABA 问题是什么？
{: #java-cas-aba }

- **CAS**：比较某个位置是否仍等于预期值，相等时才原子写入新值。它是许多无锁算法和原子类的基础，冲突时通常通过重试推进；
- **ABA 问题**：值从 A 变成 B 又回到 A，CAS 只看当前值时会误以为没有变化；
- **解决方式**：为值增加版本号或标记，例如使用带版本戳的原子引用。

CAS 并非没有代价，**高冲突下持续自旋仍会消耗大量 CPU**。
...
- **`AbortPolicy`**：抛出异常；
- **`CallerRunsPolicy`**：由提交任务的线程执行，从而减慢生产者；
- **`DiscardPolicy`**：静默丢弃当前任务；
- **`DiscardOldestPolicy`**：丢弃队列中最旧任务后重试提交。

**关键业务通常不应静默丢弃**，应选择可观测的拒绝或明确的背压方式。
...
- **堆**：线程共享，主要保存对象实例，也是垃圾回收的主要区域；
- **虚拟机栈**：线程私有，保存方法栈帧、局部变量和调用状态；
- **程序计数器**：线程私有，记录当前线程执行到的字节码位置；
- **本地方法栈**：服务于 JNI 等本地方法调用；
- **方法区**：保存类元数据、运行时常量等信息，HotSpot 通常使用 **Metaspace** 实现。

**不同区域的生命周期、容量参数和异常不同。**排查内存问题时，应先确认增长的是 Java 堆、Metaspace、线程栈还是直接内存。
...
- **标记清除**：回收不可达对象，但容易产生内存碎片；
- **复制**：把存活对象移动到另一块区域，分配简单，但需要额外空间；
- **标记整理**：把存活对象向一端移动，减少碎片，但对象移动成本较高。
...
- **G1**：把堆划分为 Region，在可预测停顿目标下分阶段回收，适合较大堆和通用服务；
- **ZGC**：把更多标记和转移工作并发化，重点降低大堆下的停顿时间；
- **Shenandoah**：同样强调并发回收和低停顿，但具体实现与支持范围不同。

ZGC 和 Shenandoah 通常会付出更多 CPU、内存或读屏障成本。选择应依据 JDK 版本、堆大小、延迟目标和真实压测，不应只按“更新的收集器一定更好”判断。
