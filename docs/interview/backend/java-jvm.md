...
对引用类型使用 `==`，比较两个引用是否指向同一对象；`equals` 表达业务上的逻辑相等。Java 约定：两个对象通过 `equals` 判断相等时，必须具有相同的 `hashCode`。

HashMap 和 HashSet 先用哈希值定位桶，再用 `equals` 确认对象。只重写 `equals` 而不重写 `hashCode`，会导致逻辑相等的对象无法正确查找或去重。作为 Key 的对象还应避免修改参与判等和哈希计算的字段。
...
checked exception 必须被捕获或声明，适合调用方能够合理恢复的外部失败。unchecked exception 继承 RuntimeException，编译器不强制处理，常用于参数、状态和程序缺陷。

异常类型应保留可行动的错误语义。权限不足、远程超时和结果未知需要不同处理方式，不能统一捕获后继续运行，也不能为了省事全部包装成同一个 RuntimeException。文件、连接等资源应使用 try-with-resources 按作用域关闭，不能依赖垃圾回收完成释放。
...
HashMap 不提供并发一致性保证，多个线程同时修改可能产生覆盖、丢失更新或不稳定读取。现代 ConcurrentHashMap 使用 CAS、桶级同步和可见性控制协调更新，读取通常不需要锁住整张表。

单个方法线程安全不代表多个方法组合具有原子性。“没有则创建”应使用 `computeIfAbsent`、`putIfAbsent` 等复合操作，而不是先 `get` 再 `put`。
...
`@Transactional` 提供事务元数据，Spring 代理在方法调用前取得或创建事务，正常结束时提交，异常符合回滚规则时回滚。

常见失效原因包括对象不是 Spring Bean、同类自调用绕过代理、方法无法被代理、异常被方法内部吞掉、异常类型不满足回滚规则，以及异步线程没有继承当前事务。默认 RuntimeException 和 Error 会回滚，普通 checked exception 默认不回滚。
