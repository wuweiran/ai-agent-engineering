...
1. **语义停滞**：计算最近几轮输出或计划的 Embedding 余弦相似度，持续高度相似且没有新增事实，说明只是在换一种说法；
2. **路径回路**：记录状态机节点序列，识别 `A → B → A` 等重复路径；
3. **信心崩塌**：模型 API 提供 Logprobs 时，观察关键决策置信度是否持续下降或剧烈波动；Logprobs 不可用时，只使用失败次数和状态变化等外部信号；
4. **动作重复或矛盾**：相同关键词重复搜索、相同参数重复调用工具，或者连续执行写入又删除等相反动作。

**跳出策略**按成本逐级升级：

- **L1 预算熔断**：限制步骤数、Token 总量、墙钟时间和单工具调用次数；
- **L2 强制重规划**：把循环轨迹和最后错误反馈给模型，要求放弃原路径并生成新计划；
- **L3 异构裁判**：对高价值任务使用不同模型判断当前路径是否仍可能推进，并给出替代方向；
- **L4 Checkpoint 回溯**：恢复到循环开始前的稳定状态，清除错误假设后换路；回溯 Agent 状态不能撤销已经发生的外部副作用；
- **L5 安全降级**：缩小目标、改为只读、返回已有结果，或进入确定性兜底流程；
- **L6 人工介入**：交付已确认事实、已完成步骤、失败轨迹和剩余问题，由用户或人工继续处理。

单一信号容易误判，生产中通常组合**语义、路径、动作和预算**检测。最大轮数负责兜底，结构化进展检测负责尽早发现。
