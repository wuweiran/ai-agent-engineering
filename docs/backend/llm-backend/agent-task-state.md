...
Agent Task 是 Agent 围绕一个用户目标持续推进、可以等待和恢复，并由系统判断是否完成的工作单元。
...
表名可以变化，恢复所需的[四种事实]({{ site.baseurl }}/docs/interview/backend/llm-application-backend/#task-run-step-tool-call)不能丢失：Task 保存目标和总体状态，Run 区分执行尝试，Step 标记稳定执行边界，Tool Call 记录副作用是否已经确认。
...
[Run 记录版本组合]({{ site.baseurl }}/docs/interview/backend/llm-application-backend/#agent-run-versioning)：这次尝试使用的模型、Prompt、工具集合和 Agent 配置版本，以及开始时间、结束原因和用量。工程师回放失败任务时，才能知道当时真正运行的是哪套组合。
...
接管 `T-301` 的 Worker 按[持久状态恢复]({{ site.baseurl }}/docs/interview/backend/llm-application-backend/#agent-worker-recovery)：
...
[租约和业务幂等解决不同问题]({{ site.baseurl }}/docs/interview/backend/llm-application-backend/#agent-lease-vs-idempotency)：租约和心跳回答“谁暂时推进任务”；Tool Call 状态与领域服务回执回答“这项业务动作能不能再次执行”。两者不能互相代替。

## Event 和 Artifact 是状态的外部投影

Task 表保存当前状态，[Event]({{ site.baseurl }}/docs/interview/backend/llm-application-backend/#agent-event-artifact)记录状态怎样变化，使前端能够展示进度并在断线后续传：
...
重要事件应与状态变化可靠关联，避免数据库已经完成而前端永远收不到完成通知。可以在同一事务中写入任务状态与本地消息表事件，再由发布器推送给订阅者。
...
凭据和用户 Token 不能进入任务记录。Trace 可以保留更丰富的排障信息，但需要脱敏、访问控制和独立保留期限。用户删除请求也要覆盖会话、任务、Artifact 和观测副本。
