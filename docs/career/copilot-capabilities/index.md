...
→ 上线后发现目标模糊会导致计划无法确认，确认后又会因邮件状态变化、部分成功或结果未知而无法完成

→ 通过**范围收敛**只澄清影响结果的条件，并基于候选邮件生成可确认的 Plan

→ 通过**执行恢复**分别处理版本冲突、部分成功和结果未知，避免过期 Plan、重复操作和错误报告成功
