...
- 邮件、日历、联系人和组织信息等多源 Context；
- Query 理解、搜索、相关性排序和会话去重；
- 用户权限、租户边界、信息来源、版本和时效；
- 长邮件线程的裁剪、摘要和 Token 预算；
- Lost in the Middle 与 Context 噪声；
- Context 质量、端到端延迟和成本之间的取舍；
- 为什么资料能够访问，不代表应该全部进入模型 Context。

如果实际链路使用关键词检索、语义搜索或 Rerank，可以在详细文章中如实展开混合检索与重排；不为了覆盖 RAG 术语虚构文档切片、向量数据库或 GraphRAG。

这条经历最适合体现完整的 **Context Engineering**：模型能力决定怎样理解信息，而 Insight Service 决定模型实际看到了什么。
