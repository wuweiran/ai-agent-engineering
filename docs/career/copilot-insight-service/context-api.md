...
Copilot Runtime 在 Extension Registry 中注册三个 Insight Extension，与 Insight Service 的三类 Context 能力对应。每个 Extension 包含名称、描述、参数 Schema 和 Handler；它以 Tool Schema 暴露给模型，模型通过 Tool Call 选择 Extension，Runtime Handler 再映射到后端 HTTP API。
...
Extension Handler 将它映射到 `POST /v1/insights/query`。
...
Extension Handler 将它映射到 `POST /v1/insights/conversations`。`query` 用来帮助长线程选择与当前问题最相关的消息，不用于重新搜索邮箱。
...
Extension Handler 将它映射到 `POST /v1/insights/calendar`。
