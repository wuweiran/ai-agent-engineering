...
Insight Service 提供需要检索、领域解析和 Context 处理的能力，服务内部不调用大模型。Copilot 模型根据用户输入、Outlook 界面 Context 和已注册工具判断是否发起 Insight Tool Call，Runtime 负责校验和执行。

```text
用户输入“找一下 Alice 上周发给我的预算更新”
→ 模型根据工具 Description 选择 search_outlook_context
→ 生成 Query“预算更新”、上周的起止时间和 Alice 的用户 UUID
→ Copilot Runtime 校验参数并执行 Tool Call
→ Insight Service 校验查询范围和权限
→ 受权限控制的邮件 Context 作为 Tool Result 返回模型
```

已知 Message ID 或 Event ID 的精确对象读取不经过 Insight Service，Runtime 直接使用 Outlook 读取工具调用 Microsoft Graph API。人员解析也不作为 Insight Service 接口暴露；模型使用已有的人员与目录 Context 取得用户 UUID，再将它作为 Query 或 Calendar 的过滤参数。

## Runtime 中的 Extension 定义

Copilot Runtime 在 Extension Registry 中注册三个 Insight Extension，与 Insight Service 的三类 Context 能力对应。每个 Extension 包含名称、Description、参数 Schema 和 Handler：Description 告诉模型什么时候使用，Schema 约束模型可以生成的参数，Handler 再把 Tool Call 映射到后端 HTTP API。

以下只展开我主要负责的邮件搜索 Extension：
...
```text
search_outlook_context Tool Call
→ Runtime 根据邮箱 Geography 选择区域
→ Handler 读取 Endpoint 配置和 /v1/insights/query 路径
→ 附加用户 Token、当前邮件 Anchor 和 Trace Context
→ Substrate 路由到同区域 Insight Service
→ Response 转换成 Tool Result 返回模型
```
...
`POST /v1/insights/query` 根据 Query、绝对时间范围和用户 UUID 搜索邮件。完整链路是：
...
1. **认证与参数校验**：从用户 Token 取得 Tenant ID、User ID 和权限范围，校验 Query、时间范围和客户端 Anchor；模型不能在请求体中覆盖身份和服务端上限；
2. **时间与人员校验**：校验 `startTime`、`endTime` 的格式、顺序和最大跨度，并检查 `participantIds` 是否是当前租户中有效的用户 UUID；
...
`POST /v1/insights/calendar` 根据 Query、绝对时间范围和用户 UUID 查询事件。服务校验时间跨度和用户 UUID，再返回与问题相关的事件。
...
Copilot Runtime 附加用户 Token、当前 Outlook 界面 Context、Request ID 和 Trace Context。Insight Service 校验时间与用户 UUID，再补充服务端执行参数：
...
模型不能填写 Tenant ID、User ID、候选数量或服务端 Token 上限。用户 UUID 必须来自当前客户端 Context 或此前可信的人员查询结果，不能由模型凭空编造。Insight Service 从用户 Token 的 Claims 中取得用户与租户身份，并在查询和结果读取时执行对象级授权。
...
用户身份来自 Copilot Runtime 转发的 Microsoft Entra ID 用户 Token，不来自模型参数。Insight Service 验证 Token Claims，并通过 On-Behalf-Of 流程取得 Outlook 下游 Token，使搜索继续以用户身份执行对象级授权。候选返回后，服务再校验租户、邮箱和对象边界。完整权限链路见 [Outlook 邮件 RAG]({{ site.baseurl }}/docs/career/copilot-insight-service/mail-rag/)。

请求还受到时间范围、候选数量和 Token 预算限制。模型不能通过省略时间条件或扩大返回数量绕过服务端上限；用户 UUID 只用于缩小查询范围，不会扩大用户对邮箱或日历对象的访问权限。
